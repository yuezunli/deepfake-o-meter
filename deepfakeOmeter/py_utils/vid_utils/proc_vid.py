...
def convert_vid(input_path, output_path, codec='H264'):
    # ffmep must be installed. If H264 is need, libx264 is required
    if codec == 'H264':
        cmd = 'ffmpeg -i {} -vcodec libx264 {}'.format(input_path, output_path)
        os.system(cmd)
...
def gen_vid_from_folder(video_path, img_dir, ext, fps, width=None, height=None):
    imgs_path = sorted(glob(img_dir + '/*.' + ext))
