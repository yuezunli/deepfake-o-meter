...
def get_face_loc_landmarks(im, face_detector, lmark_predictor, scale=0):
    """ get face locations, color order of images is rgb """
    faces = face_detector(np.uint8(im), scale)
    face_list = []
    if faces is not None or len(faces) > 0:
        for i, d in enumerate(faces):
            try:
                ...
                points = shape_to_np(lmark_predictor(im, d))
            except:
                loc = [d.rect.left(), d.rect.top(), d.rect.right(), d.rect.bottom()]
                points = shape_to_np(lmark_predictor(im, d.rect))
...
def get_face_mask_v2(shape, face_cache):
    mat, landmarks = face_cache
    size = 256
    padding = (0, 0)
    landmarks_aligned = get_2d_aligned_landmarks(face_cache, size, padding)
...
def get_face_mask_v3(shape, face_cache, outer_inner_ratio=0.5):
    mat, landmarks = face_cache
    size = 256
    padding = (0, 0)
    landmarks_aligned = get_2d_aligned_landmarks(face_cache, size, padding)
