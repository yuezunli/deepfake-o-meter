...
class VideoReader:
    ...
    def read_frames(self, path, num_frames=32, jitter=0, seed=None):
...
def init_model():
    wightpath =  os.path.dirname(__file__) + '/weights/'
...
def predict(models, img):
    prediction = predict_on_image(faces=img, input_size=380, models=models, strategy=np.mean, apply_compression=False)
    return prediction
...
# altred by cz
def predict_on_image(faces, input_size, models, strategy=np.mean,
